...
$query = 'rabbitmq_queue_messages_ready{queue="cola_telegram"}';
...
$rawOutput = shell_exec("docker images -q img_consumer_telegram");
$imageExists = trim($rawOutput ?? '');
if ($imageExists === '') {
    echo "Construyendo imagen base 'img_consumer_telegram'...\n";
    $projectRoot = realpath(__DIR__ . '/../consumer_telegram');
    $output = shell_exec("docker build -t img_consumer_telegram " . escapeshellarg("{$projectRoot}") . " 2>&1");
    echo $output;
}

while (true) {
    echo "\n--- Verificando cola 'telegram' ---\n";
    ...
    $existing = shell_exec("docker ps --filter 'name=consumer_telegram_' --format '{{.Names}}'");
    ...
    if ($neededConsumers > $runningCount) {
        ...
        for ($i = $runningCount + 1; $i <= $neededConsumers; $i++) {
            $name = "consumer_telegram_$i";
            echo "Creando $name...\n";

            $commands[] = "docker run -d --name $name --network rabbitmq_network img_consumer_telegram &";
