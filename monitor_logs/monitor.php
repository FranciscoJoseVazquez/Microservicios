...
$rawOutput = shell_exec("docker images -q img_consumer_logs");
$imageExists = trim($rawOutput ?? '');
if ($imageExists === '') {
    echo "Construyendo imagen base 'img_consumer_logs'...\n";
    $projectRoot = realpath(__DIR__ . '/../consumer_logs');
    $output = shell_exec("docker build -t img_consumer_logs " . escapeshellarg("{$projectRoot}") . " 2>&1");
    echo $output;
}

while (true) {
    echo "\n--- Verificando cola 'logs' ---\n";
